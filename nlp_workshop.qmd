...
Then we feed that to the LDA function to optimize its topic clusters.  But first, we need to know how many clusters we would like it to find!  

### Perplexity score

From https://www.mathworks.com/help/textanalytics/ug/choose-number-of-topics-for-LDA-model.html:
...
This code chunk runs a cross validation on a subset of the overall corpus, trying LDA on a training set for a range of k values, then testing perplexity against a test set,
...
if(!file.exists(k_optim_file)) {
  ...
  test_ids <- refs_subset %>%
    slice_sample(prop = .2, replace = FALSE) %>%
    .$doc_id
  
  train_ids <- refs_subset %>%
  ...
  test_dtm <- abstr_clean_df %>%
    filter(doc_id %in% test_ids) %>%
    cast_dtm(term = lemma, document = doc_id, value = n)
  train_dtm <- abstr_clean_df %>%
    filter(doc_id %in% train_ids) %>%
    cast_dtm(term = lemma, document = doc_id, value = n)

  k_vec <- c(2:10, seq(12, 20, 2), seq(25, 40, 5))
  
  perp_fxn <- function(k, train_dtm, test_dtm) {
    message('Processing LDA for k = ', k, '...')
    
    lda_out <- topicmodels::LDA(train_dtm, k = k)
    perp <- perplexity(object = lda_out, newdata = test_dtm)
    
    return(data.frame(k = k, perplexity = perp))
  }
  
  perp_vs_k_df <- purrr::map_df(k_vec, perp_fxn, 
                                train_dtm = train_dtm, test_dtm = test_dtm) %>%
    bind_rows() %>%
    arrange(k)
...
ggplot(perp_vs_k_df, aes(x = k, y = perplexity)) +
  ...
  geom_vline(xintercept = 6, linewidth = .5, linetype = 'dashed', color = hcl.colors(1)) +
  theme_minimal() +
...
This method is really slow though, and doesn't seem super definitive in this case.  There

### `ldatuning` package

The `ldatuning` package runs four different metrics to identify a good number of topics.
...
In the perplexity code above, we already see how to do the LDA.
